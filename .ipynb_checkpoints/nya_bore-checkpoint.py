...
def define_discriminator(image_shape):
	...
	# compile model with updated Adam optimizer
...
def train(d_model, g_model, gan_model, train_datagen, n_epochs=100, n_batch=1):
    # determine the output square shape of the discriminator
    n_patch = d_model.output_shape[1]
    # calculate the number of batches per training epoch
    bat_per_epo = len(train_datagen)
    # calculate the number of training iterations
    n_steps = bat_per_epo * n_epochs
    
    # manually enumerate epochs
    for i in range(n_steps):
        # select a batch of real samples from the generator
        X_realA, X_realB = train_datagen.__getitem__(i % bat_per_epo)[0]# Proper tuple unpacking
        
        # generate labels for real samples
        y_real = np.ones((n_batch, n_patch, n_patch, 1))  # You can adjust this based on your discriminator's output
        ...
        if (i+1) % (bat_per_epo * 10) == 0:
            summarize_performance(i, g_model, train_datagen)